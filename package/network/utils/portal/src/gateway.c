...
/** @internal
  ...
  @author Copyright (C) 2008 Paul Kube <portal@kokoro.ucsd.edu>
 */
...
main_loop(void)
{
	...
	/* Set the time when portal started */
	if (!started_time) {
	...
	httpdAddCContent(webserver, "/", "", 0, NULL, http_portal_callback_index);
	httpdAddCWildcardContent(webserver, config->authdir, NULL, http_portal_callback_auth);
	httpdAddCWildcardContent(webserver, config->denydir, NULL, http_portal_callback_deny);
	httpdAddC404Content(webserver, http_portal_callback_404);

	/* Reset the firewall (cleans it, in case we are restarting after portal crash) */
	...
	result = pthread_create(&tid, NULL, thread_ptlctl, (void *)safe_strdup(config->ptlctl_sock));
	if (result != 0) {
		debug(LOG_ERR, "FATAL: Failed to create thread_ptlctl - exiting");
		termination_handler(0);
	}
	pthread_detach(tid);
