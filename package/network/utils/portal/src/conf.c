...
/** @file conf.c
  ...
  @author Copyright (C) 2007 Paul Kube <nodogportal@kokoro.ucsd.edu>
 */
...
config_init(void)
{
	...
	config.portalpage = safe_strdup(DEFAULT_SPLASHPAGE);
	...
	config.ptlctl_sock = safe_strdup(DEFAULT_NDSCTL_SOCK);
	...
	while (fgets(line, MAX_BUF, fd)) {
		...
		*p1 = '\0';
		...
		case oWebRoot:
			...
			break;
		case oSplashPage:
			config.portalpage = safe_strdup(p1);
			break;
		case oImagesDir:
		...
		case oRedirectURL:
			...
			break;
		case oPtlctlSocket:
			free(config.ptlctl_sock);
			config.ptlctl_sock = safe_strdup(p1);
			break;
