...
/** @file http.c
  ...
  @author Copyright (C) 2007 Paul Kube <portal@kokoro.ucsd.edu>
 */
...
/** The 404 handler is one way a client can first hit portal.
 */
void
http_portal_callback_404(httpd *webserver, request *r)
{
	debug(LOG_INFO, "Capturing as 404 request from %s for [%s%s]",
		  r->clientAddr, r->request.host, r->request.path);

	http_portal_first_contact(r);
}

/** The index handler is one way a client can first hit portal.
 */
void
http_portal_callback_index(httpd *webserver, request *r)
{
 ...
 *  Add the client to the client list and serves the portal page.
 */
void
http_portal_first_contact(request *r)
{
	t_client *client;
	...
	client = http_portal_add_client(r);
	/* http_portal_add_client() should log and return null on error */
	if(!client) return;
	...
	*/
	...
	redir = http_portal_make_redir(origurl);

	/* Create authtarget with all needed info */
	authtarget = http_portal_make_authtarget(client->token,redir);

	free(origurl);

	if(config->authenticate_immediately) {
		/* Don't serve portal, just authenticate */
		http_portal_callback_action(r,authtarget,AUTH_MAKE_AUTHENTICATED);
		...
		/* Serve the portal page (or redirect to remote authenticator) */
serve_portal:
		free(data);
		http_portal_serve_portal(r,authtarget, client, NULL);
	}

	http_portal_free_authtarget(authtarget);
}

/** The multipurpose authentication action handler
 */
void
http_portal_callback_action(request *r,
								 t_auth_target *authtarget,
								 t_authaction action)
{
	t_client	*client;
...
void
http_portal_callback_auth(httpd *webserver, request *r)
{
	s_config *config;
	t_client  *client;
	t_auth_target *authtarget;
	...
	authtarget = http_portal_decode_authtarget(r);
	config = config_get_config();
	...
	if (config->bin_voucher && ((authtarget->voucher) || (config->force_voucher))) {
		...
		http_portal_callback_action(r,authtarget,AUTH_MAKE_AUTHENTICATED);
		client->added_time = time(NULL) - (config->checkinterval * config->clientforceout) + seconds;
	} else if(http_portal_check_userpass(r,authtarget)) {
		http_portal_callback_action (r,authtarget,AUTH_MAKE_AUTHENTICATED);
	} else {
		/* Password check failed; just serve them the portal page again */
serve_portal:
		if (data) {
			msg = strchr(data, ' ');
			if (msg)
		...
		http_portal_serve_portal(r,authtarget,client,msg);
		free(data);
	}
...
void
http_portal_callback_deny(httpd *webserver, request *r)
{
	t_auth_target *authtarget;

	/* Get info we need from request, and do action */
	authtarget = http_portal_decode_authtarget(r);
	http_portal_callback_action (r,authtarget,AUTH_MAKE_DEAUTHENTICATED );
	http_portal_free_authtarget(authtarget);
}
 ...
 */
t_client *
http_portal_add_client(request *r)
{
	t_client	*client;
	LOCK_CLIENT_LIST();
...
void
http_portal_redirect_remote_auth(request *r, t_auth_target *authtarget)
{
	char *remoteurl;
	...
	safe_asprintf(&remoteurl, "%s?gateway=%s&authaction=%s&redir=%s&tok=%s",
				  ...
				  encredir,
				  enctoken);
	http_portal_redirect(r, remoteurl);
	free(encgateway);
	free(encauthaction);
	free(encredir);
	free(enctoken);
...
/* Pipe the portal page from the portal page file,
 * or redirect to remote authenticator as required.
 */
void
http_portal_serve_portal(request *r, t_auth_target *authtarget, t_client *client, const char error_msg[])
{
	char *tmpstr;
	char line [MAX_BUF];
	char *portalfilename;
	...
	if(config->remote_auth_action) {
		/* Redirect to remote auth server instead of serving local portal page */
		http_portal_redirect_remote_auth(r, authtarget);
		return;
	}

	/* Set variables; these can be interpolated in the portal page text. */
	if (error_msg)
	...
	/* Pipe the portal page from its file */
	safe_asprintf(&portalfilename, "%s/%s", config->webroot, config->portalpage );
	debug(LOG_INFO,"Serving portal page %s to %s",
 ...
 */
void
http_portal_serve_info(request *r, const char title[], const char content[])
{
	char *abspath;
	char line [MAX_BUF];
...
void
http_portal_redirect(request *r, const char url[])
{
	char *header;
 ...
 * See http_portal_make_authtarget().
 * The struct should be freed by http_portal_free_authtarget().
 */
t_auth_target *
http_portal_decode_authtarget(request *r)
{
	httpVar *var;
	t_auth_target *authtarget;
	...
	authtarget = http_portal_make_authtarget(token,redir);
 ...
 */
const char*
http_portal_make_redir(const char origurl[])
{
	s_config *config;
	config = config_get_config();
 ...
 */
t_auth_target*
http_portal_make_authtarget(const char token[], const char redir[])
{
	char *encodedredir;
	char *encodedtok;
	t_auth_target* authtarget;
...
void
http_portal_free_authtarget(t_auth_target* authtarget)
{
 ...
 */
int
http_portal_check_userpass(request *r, t_auth_target *authtarget)
{
	s_config *config;
	t_client  *client;
	config = config_get_config();
